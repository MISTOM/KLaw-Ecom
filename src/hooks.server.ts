...
export const handle: Handle = async ({ event, resolve }) => {
    ...
    if (token) {
        try {
            ...
            if (decodedToken.exp && decodedToken.exp - currentTime < 60 && refreshToken) { //expires in 1 minute
    ...
    if (event.url.pathname.startsWith('/admin')) {
        ...
        if (!adminRole || event.locals.user?.role !== adminRole.id) {
            console.log('user not admin' + event.locals.user?.role);
