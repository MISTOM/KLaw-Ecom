...
interface Args {
	title: string;
	input: string; // pdf path OR directory of images
	docId?: number; // allow reuse / overwrite? (Not recommended). If omitted new DB row.
	pagesPattern?: string; // glob-like pattern if pre-split, default *.png
	dryRun?: boolean;
}
...
async function pdfToImages(pdfPath: string, workDir: string): Promise<string[]> {
...
async function collectImages(input: string, pagesPattern = '.png'): Promise<string[]> {
...
function encryptContentKey(masterKey: Buffer, contentKey: Buffer): string {
...
function decryptWrappedKey(masterKey: Buffer, wrapped: string): Buffer {
...
function encryptPage(contentKey: Buffer, pageBytes: Buffer): Buffer {
...
async function main() {
	...
	const tempDir = path.join(process.cwd(), '.clear', crypto.randomUUID());
	if (isPdf) {
		if (args.dryRun) {
